...
class BenchmarkResult:
    ...
    roc_data: dict = field(default_factory=dict)  # {fpr, tpr, auc}
...
class BenchmarkEvaluator:
    ...
    def evaluate_detector(self, detector) -> BenchmarkResult:
        ...
        for atype in set(self.dataset.anomaly_types.values()):
            ...
            # anomaly only recall (해당 유형의 anomaly만 보기)
            type_true = np.ones(len(type_indices))
        ...
        for diff in ["easy", "medium", "hard"]:
            ...
            diff_true = np.ones(len(diff_indices))
